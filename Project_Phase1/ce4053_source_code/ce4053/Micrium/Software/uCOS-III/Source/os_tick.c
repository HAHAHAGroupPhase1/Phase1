...
void  OS_TickListUpdate (void)
{
    ...
    if(OSTickCtr == pow(256, sizeof(OS_TICK)) - 1)
    {
      OverFlowCtr++;          /* Increse OverflowCounter */
      OSTimeSet(0,&err);        /* Reset the system clock  */
    }
    else 
    {
    ...
    //look for the current Tick in the data structure
     node* TaskToDispatch = search((node*) tree, (OS_TICK ) OSTickCtr);
     if(TaskToDispatch != NULL)
     {
       OS_RdyListInsertTail(TaskToDispatch->p_tcb);
     }
